...
def init_obj(opt, logger, *args, default_file_name='default file', given_module=None, init_type='Network', **modify_kwargs):
    """
  Look for a function handle with the name "name" in the configuration,
    and return the instance initialized with the corresponding parameters.
    """
    if opt is None or len(opt)<1:
        logger.info('Option is None when initialize {}'.format(init_type))
        return None
    
    ''' The default format is with a name key dict'''
    if isinstance(opt, str):
        opt = {'name': opt}
        logger.warning('Config is a str, converts to a dict {}'.format(opt))

    name = opt['name']
    ''' name can be a list, which represents the file and class name of the function '''
    ...
    try:
        ...
        ''' Use args to import a class or function '''
...
def parse(args):
    ...
    '''Replace the configuration context with args '''
    ...
    ''' Change the folder relative hierarchy '''
