...
def load_UNet_state_dict(path, device):
    ...
    has_denoise_fn = any(key.startswith('denoise_fn.') for key in checkpoint.keys())
    for key, value in checkpoint.items():
        if has_denoise_fn:
            if key.startswith('denoise_fn.'):
                unet_state_dict[key.replace('denoise_fn.', '', 1)] = value
        else:
            if key.startswith('module.'):
                unet_state_dict[key.replace('module.', '', 1)] = value
            else:
...
def make_argument_parser():
    ...
    parser.add_argument("--base_checkpoint", type=str, default="320checkpoints/200_Network.pth")
    parser.add_argument("--data_root", type=str, default="train_320")
    ...
    parser.add_argument("--num_iters", type=int, default=45170)
    ...
    parser.add_argument("--disable_ddp", action="store_true", help="禁用分布式训练")
    parser.add_argument("--local_rank", type=int, default=-1, help="自动设置的DDP参数")
    return parser
