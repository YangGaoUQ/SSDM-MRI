...
def tensor2nii(tensor, out_type=np.float32, min_max=(0, 1)):
    # tensor = tensor.clamp_(*min_max)
    n_dim = tensor.dim()
    if n_dim == 4:
        n_img = len(tensor)
...
def postprocess_png(images):
	return [tensor2img(image) for image in images]

def postprocess_nii_npy(images):
	return [tensor2nii(image) for image in images]
