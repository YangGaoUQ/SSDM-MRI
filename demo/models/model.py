...
class Palette(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, **kwargs):
        ''' 必须使用 kwargs 初始化 BaseModel '''
        super(Palette, self).__init__(**kwargs)

        ''' 网络、Dataloder、优化器、损失等. '''
        ...
        ''' networks 可以是一个列表，如果使用多个 GPU，则必须通过self.set_device函数进行转换. '''
    ...
    def set_input(self, data):
        ...
        self.coil_sen=data.get('coil_sen')
        self.path = data['path']
    ...
    def get_current_visuals(self, phase='train'):
        dict = {
            ...
            'cond_image': (self.cond_image.detach()[:].float().cpu()) / 2
        }
        if phase != 'train':
            dict.update({
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            loss = self.netG(self.gt_image, self.cond_image, mask=self.mask,sub_kspace=self.sub_kspace,coil_sen=self.coil_sen)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if torch.cuda.device_count() > 1:
                    self.output, self.visuals = self.netG.module.restoration(y_cond=self.cond_image,mask=self.mask,sample_num=self.sample_num,sub_kspace=self.sub_kspace,coil_sen=self.coil_sen)
                else:
                    self.output, self.visuals = self.netG.restoration(y_cond=self.cond_image, mask=self.mask,sample_num=self.sample_num,sub_kspace=self.sub_kspace,coil_sen=self.coil_sen)
