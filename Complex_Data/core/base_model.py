...
class BaseModel():
    def __init__(self, opt, phase_loader, val_loader, metrics, logger, writer):
        """ An initialization model with basic inputs from the __init__(**kwargs) function in the inheritance class """
        self.opt = opt
        ...
        ''' logger added to the log file, this only applies to GPU 0. Writer and result file for TensorBoard '''
        self.logger = logger
        self.writer = writer
    ...
    def train(self):
        while self.epoch <= self.opt['train']['n_epoch'] and self.iter <= self.opt['train']['n_iter']:
            ...
            if self.opt['distributed']:
                ''' Sets the epoch of this sampler. When :attr:'shuffle=True', this ensures that all replicas use a different random order for each epoch '''
            ...
            ''' Print the recorded information to the screen andTensorBoard '''
    ...
    def save_network(self, network, network_label):
        """ Save the network structure and only work on GPU 0 """
        if self.opt['global_rank'] !=0:
            return
    ...
    def save_training_state(self):
        """ Saves training state during training and only works on GPU 0 """
        if self.opt['global_rank'] !=0:
            return
    ...
    def resume_training(self):
        """ The recovery optimizer and scheduler train and only work when the stage is enabled for testing or recovery training """
