...
class MRI_Reconstruction(BaseModel):
    def __init__(self, networks, losses, sample_num, task, optimizers, ema_scheduler=None, **kwargs):
        ''' Must be initialized with kwargs BaseModel '''
        super(MRI_Reconstruction, self).__init__(**kwargs)

        ''' Networking, Dataloder, Optimizer, Loss, etc. '''
        ...
        ''' networks can be a list that must be converted through a self.set_device function if multiple GPUs are used. '''
        ...
        ''' You can override it in the inherited class for more information logging records '''
    ...
    def set_input(self, data):
        ''' Must be used in tensors set_device '''
        self.sub_kspace = data.get('sub_kspace')
        self.cond_image = self.set_device(data.get('cond_image'))
        self.gt_image = self.set_device(data.get('gt_image'))
        self.mask = data.get('mask')
        self.path = data['path']
        self.batch_size = len(data['path'])
        self.norm_max = data.get('norm_max')  # 加载 norm_max
    ...
    def train_step(self):
        ...
        for train_data in tqdm.tqdm(self.phase_loader):
            ...
            if self.iter % self.opt['train']['log_iter'] == 0:
                ...
                #     value = complex_tensor_to_abs(value)
    ...
    def test(self):
        ...
        with torch.no_grad():
            for phase_data in tqdm.tqdm(self.phase_loader):
                ...
                if torch.cuda.device_count()>1:
                    self.output, self.visuals = self.netG.module.restoration(self.cond_image, sample_num=self.sample_num,mask=self.mask,sub_kspace=self.sub_kspace,norm_max=self.norm_max)
                else:
                    self.output, self.visuals = self.netG.restoration(self.cond_image, sample_num=self.sample_num,mask=self.mask,sub_kspace=self.sub_kspace,norm_max=self.norm_max)

                self.iter += self.batch_size
                self.writer.set_iter(self.epoch, self.iter, phase='test')
                for met in self.metrics:
                    key = met.__name__
                    device = self.output.device  # 获取 y_t 的设备
                    norm_max = self.norm_max.to(device)
                    output = self.output /norm_max
                    value = met(self.gt_image, output)
        ...
        ''' Saves the recorded information to a log dictionary '''
        test_log.update({'epoch': self.epoch, 'iters': self.iter})

        ''' Print the recorded information to the screen and TensorBoard '''
        for key, value in test_log.items():
            self.logger.info('{:5s}: {}\t'.format(str(key), value))

    def load_networks(self):
        """ Save the pretrained model and training state, executed only on GPU 0. """
        if torch.cuda.device_count()>1:
    ...
    def save_everything(self):
        """ Load the pre-trained model and training status. """
        if torch.cuda.device_count()>1:
            netG_label = self.netG.module.__class__.__name__
        else:
            netG_label = self.netG.__class__.__name__
